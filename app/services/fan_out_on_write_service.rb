...
class FanOutOnWriteService < BaseService
  ...
  def call(status)
    ...
    return if status.account.silenced? || !status.public_visibility? && !(status.unlisted_visibility? && !(status.reply? && status.in_reply_to_account_id != status.account_id)) || status.reblog?
